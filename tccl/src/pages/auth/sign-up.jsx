...
export function SignUp() {
	...
	const handleSubmit = async (event) => {
		...
		if (userData.password_hash !== password2) {
		...
		try {
			// Hash the password
			const hashedPassword = await bcrypt.hash(userData.password_hash, 10); // 10 is the saltRounds

			// Update the user data with the hashed password
			setUserData({ ...userData, password_hash: hashedPassword });
				...
				<Card className="absolute top-2/4 left-2/4 w-full max-w-[24rem] -translate-y-2/4 -translate-x-2/4">
					...
					<CardBody className="flex flex-col gap-4">
						<form className="grid lg:grid-cols-1 gap-6" onSubmit={handleSubmit}>
							...
							<Input name="password_hash" type="password" label="Password" size="lg" onChange={handleInputChange} />
							<Input
								name="password2"
								type="password"
